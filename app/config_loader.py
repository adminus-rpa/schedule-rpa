...
DEFAULTS: dict[str, Any] = {
    ...
    "display": {
        ...
        "room_replacements": {
            ...
            # Если аудитория пустая или прочерк — это ЭИОС
            r"\A\s*[-—–]*\s*\Z": "ЭИОС",

            # Те же правила для ЭИОС, так как иногда диспетчеры пишут ЭИОС в колонку кабинета
            r"\A\s*(?:(?:лекция|лекц|лек|практика|практ|семинар|сем|конс|кол|лаб|сам|пр|л|с|к|н)\.?[\s/\\\-\.]*){0,3}Электронн[а-я]*[\s\-]+информационн[а-я]*[\s\-]+образовательн[а-я]*[\s\-]+сред[а-я]*[\.\s]*\Z": "ЭИОС",
            r"\A\s*(?:(?:лекция|лекц|лек|практика|практ|семинар|сем|конс|кол|лаб|сам|пр|л|с|к|н)\.?[\s/\\\-\.]*){0,3}(?:Э\.?\s*И\.?\s*О\.?\s*С\.?|ЭИО[\s\-]?С|ЭИ[\s\-]?ОС)[\.\s]*\Z": "ЭИОС",
            r"\bЭ\.\s*И\.\s*О\.\s*С\.?(?=\W|$)": "ЭИОС",
            r"\bЭ\s+И\s+О\s+С\b": "ЭИОС",
            r"\b(?:ЭИО[\s\-]С|ЭИ[\s\-]ОС)\b": "ЭИОС",
            r"\b[Ээ][Ии][Оо][Сс]\b": "ЭИОС",
